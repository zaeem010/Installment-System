...
namespace AR_IS.Controllers
{
    ...
    public class SaleController : Controller
    {
        ...
        public ActionResult Save(SaleMaster SaleMaster, TranscationDetail TranscationDetail, int[] prid, string[] productname, decimal[] saleprice, decimal[] qty, decimal[] nettotal)
        {
            string varDirection = "";
            if (SaleMaster.Id == 0)
            {
                ...
                SaleMaster.Vtype = "SINV";
                for (int i = 0; i < prid.Count(); i++)
                {
                    _context.Database.ExecuteSqlCommand("INSERT INTO SaleDetails (Invid, Itemid, ItemName, SP, Qty, NetTotal, Date, Vtype, Comid,CTN,ItemUnit)  VALUES ('" + SaleMaster.Invid + "','" + prid[i] + "','" + productname[i] + "','" + saleprice[i] + "','" + qty[i] + "','" + nettotal[i] + "','" + SaleMaster.Date + "','SINV','" + Session["Company"] + "',0,0)");
                }
            ...
            {
                _context.Database.ExecuteSqlCommand("DELETE FROM SaleMasters  WHERE (Vtype = 'SINV') AND (Invid = '" + SaleMaster.Invid + "') AND (Comid = '" + Session["Company"] + "') ");
                _context.Database.ExecuteSqlCommand("DELETE FROM SaleDetails  WHERE (Vtype = 'SINV') AND (Invid = '" + SaleMaster.Invid + "') AND (Comid = '" + Session["Company"] + "') ");
                _context.Database.ExecuteSqlCommand("DELETE FROM TranscationDetails  WHERE (Vtype = 'SINV') AND (Invid = '" + SaleMaster.Invid + "') AND (Comid = '" + Session["Company"] + "') ");
                _context.tbl_SaleMaster.Add(SaleMaster);
                SaleMaster.Comid = Convert.ToInt32(Session["Company"]);
                SaleMaster.Vtype = "SINV";
                for (int i = 0; i < prid.Count(); i++)
            ...
            return RedirectToAction(varDirection, "Sale");
        }
        public ActionResult Edit(int Invid, TranscationDetail TranscationDetail)
        {
            var ViewModel = new SaleVM
            {
                SaleDetail_list = _context.Database.SqlQuery<SaleDetail>("SELECT  * FROM   SaleDetails  WHERE (Vtype='SINV')  AND  (Invid = '" + Invid + "') AND (Comid = '" + Session["Company"] + "')").ToList(),
                SaleMaster = _context.Database.SqlQuery<SaleMaster>("SELECT  * FROM   SaleMasters  WHERE (Vtype='SINV')  AND  (Invid = '" + Invid + "') AND (Comid = '" + Session["Company"] + "')").FirstOrDefault(),
                Cus_list = _context.Database.SqlQuery<Customer>("SELECT * FROM   Customers WHERE (Comid = '" + Session["Company"] + "')").ToList(),
                Prod_list = _context.Database.SqlQuery<Product>("SELECT *   FROM   Products  WHERE ( Comid = '" + Session["Company"] + "')  ORDER BY Id").ToList(),
                TranscationDetail = _context.Database.SqlQuery<TranscationDetail>("SELECT *  FROM   TranscationDetails  WHERE (Invid = '"+Invid+"') AND (Comid = '" + Session["Company"] + "') AND (Vtype = 'SINV') AND (TransDes = 'Customer')").FirstOrDefault(),
                SaleRecent = _context.Database.SqlQuery<SaleVMQ>("SELECT SaleMasters.Id, SaleMasters.Invid, SaleMasters.Phone, SaleMasters.Date, SaleMasters.CargoName, SaleMasters.CargoCharges, SaleMasters.NetAmount, SaleMasters.DiscountAmount, SaleMasters.GrandTotal, SaleMasters.Total, SaleMasters.BTotal, SaleMasters.Vtype, SaleMasters.Comid, SaleMasters.AccountNo, SaleMasters.Rtotal, Customers.Name FROM SaleMasters INNER JOIN Customers ON SaleMasters.AccountNo = Customers.AccountNo WHERE(SaleMasters.Comid = '" + Session["Company"] + "') AND (SaleMasters.Vtype = 'SINV') AND (Customers.Comid = '" + Session["Company"] + "') ORDER BY SaleMasters.Invid").ToList(),
            };
        ...
        {
            var saleprice = _context.Database.SqlQuery<Product>("SELECT *  FROM   Products WHERE (Id = '" + id + "')").ToList();
